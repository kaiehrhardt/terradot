## [1.4.11](https://github.com/kaiehrhardt/terradot/compare/1.4.10...1.4.11) (2026-07-09)


### Bug Fixes

* **deps:** update dependency @hpcc-js/wasm to v2.34.5 ([#48](https://github.com/kaiehrhardt/terradot/issues/48)) ([88c6493](https://github.com/kaiehrhardt/terradot/commit/88c6493c04acab55fb54130d42c439a068c53836))

## [1.4.10](https://github.com/kaiehrhardt/terradot/compare/1.4.9...1.4.10) (2026-07-06)


### Bug Fixes

* **deps:** update all dependencies ([#47](https://github.com/kaiehrhardt/terradot/issues/47)) ([9c2e3e9](https://github.com/kaiehrhardt/terradot/commit/9c2e3e9385b5654dd2c0c35386f9bf9ab6d0dfdd))

## [1.4.9](https://github.com/kaiehrhardt/terradot/compare/1.4.8...1.4.9) (2026-07-02)


### Bug Fixes

* **deps:** update all dependencies ([#46](https://github.com/kaiehrhardt/terradot/issues/46)) ([c556996](https://github.com/kaiehrhardt/terradot/commit/c5569968ab002d6a50c1167a6586ebb4b0408ee3))

## [1.4.8](https://github.com/kaiehrhardt/terradot/compare/1.4.7...1.4.8) (2026-06-22)


### Bug Fixes

* **deps:** update all dependencies ([#40](https://github.com/kaiehrhardt/terradot/issues/40)) ([6089b5b](https://github.com/kaiehrhardt/terradot/commit/6089b5b60cf2051b553b22a51d2d0ded2543e1d6))

## [1.4.7](https://github.com/kaiehrhardt/terradot/compare/1.4.6...1.4.7) (2026-06-08)


### Bug Fixes

* **deps:** update all dependencies ([#35](https://github.com/kaiehrhardt/terradot/issues/35)) ([b811afa](https://github.com/kaiehrhardt/terradot/commit/b811afa24837ee85e340a9b0f8726db6307eda22))

## [1.4.6](https://github.com/kaiehrhardt/terradot/compare/1.4.5...1.4.6) (2026-06-04)


### Bug Fixes

* **deps:** update all dependencies ([#34](https://github.com/kaiehrhardt/terradot/issues/34)) ([5928650](https://github.com/kaiehrhardt/terradot/commit/5928650e6c416c948c95e102f4438e0b68194e2e))

## [1.4.5](https://github.com/kaiehrhardt/terradot/compare/1.4.4...1.4.5) (2026-05-28)


### Bug Fixes

* **deps:** update dependency @hpcc-js/wasm to v2.33.8 ([#33](https://github.com/kaiehrhardt/terradot/issues/33)) ([e654822](https://github.com/kaiehrhardt/terradot/commit/e654822ae3839cfcc5a305e19f5d5e2157e7df8c))

## [1.4.4](https://github.com/kaiehrhardt/terradot/compare/1.4.3...1.4.4) (2026-05-25)


### Bug Fixes

* **deps:** update all dependencies ([#31](https://github.com/kaiehrhardt/terradot/issues/31)) ([98b97e3](https://github.com/kaiehrhardt/terradot/commit/98b97e31853fe697afe7df205a50f359682865cc))

## [1.4.3](https://github.com/kaiehrhardt/terradot/compare/1.4.2...1.4.3) (2026-05-19)


### Bug Fixes

* always select all modules by default when no URL state present ([2ffca6e](https://github.com/kaiehrhardt/terradot/commit/2ffca6ec24e12a5058c6a7a1d22bde79f7c5c7b6))

## [1.4.2](https://github.com/kaiehrhardt/terradot/compare/1.4.1...1.4.2) (2026-05-19)


### Bug Fixes

* **deps:** pin dependencies ([d934bd6](https://github.com/kaiehrhardt/terradot/commit/d934bd6f71b0f84c09378efbd7370594c1678535))

## [1.4.1](https://github.com/kaiehrhardt/terradot/compare/1.4.0...1.4.1) (2026-05-19)


### Bug Fixes

* **lint:** resolve ref-during-render and prettier errors in App.tsx ([deec0e8](https://github.com/kaiehrhardt/terradot/commit/deec0e8de391524b3fd891e4a4fa2211b656877f))

# [1.4.0](https://github.com/kaiehrhardt/terradot/compare/1.3.2...1.4.0) (2026-05-19)


### Features

* add URL-based state sharing ([c80be97](https://github.com/kaiehrhardt/terradot/commit/c80be975d91c35c747e93c7834dc78061c078a1e))

## [1.3.2](https://github.com/kaiehrhardt/terradot/compare/1.3.1...1.3.2) (2026-05-18)


### Bug Fixes

* **deps:** update all dependencies ([19129ae](https://github.com/kaiehrhardt/terradot/commit/19129ae994447c6f12a317cdc080e5ef3f0f7806))
* linter issues ([dd83d95](https://github.com/kaiehrhardt/terradot/commit/dd83d95fce74b02ee0efa57a1ec1ba5a9758b9be))

## [1.3.1](https://github.com/kaiehrhardt/terradot/compare/1.3.0...1.3.1) (2026-04-13)


### Bug Fixes

* prevent stuck loading spinner and incomplete reset in handleLoadExample ([a465c66](https://github.com/kaiehrhardt/terradot/commit/a465c66ff916337eae9cb42fbea6eba1879539c5))

# [1.3.0](https://github.com/kaiehrhardt/terradot/compare/1.2.5...1.3.0) (2026-04-13)


### Features

* add direct-only neighbor toggle for node selection ([ab79ebd](https://github.com/kaiehrhardt/terradot/commit/ab79ebdb3caea169cda496684c5a8b29202b2f98)), closes [#26](https://github.com/kaiehrhardt/terradot/issues/26)

## [1.2.5](https://github.com/kaiehrhardt/terradot/compare/1.2.4...1.2.5) (2026-04-10)


### Bug Fixes

* remove useless viewBox reassignment in catch block ([80e673f](https://github.com/kaiehrhardt/terradot/commit/80e673f612b0b85b886d37d178b4be07ac11183d))

## [1.2.4](https://github.com/kaiehrhardt/terradot/compare/1.2.3...1.2.4) (2026-04-10)


### Bug Fixes

* switch back to node runtime ([f745a89](https://github.com/kaiehrhardt/terradot/commit/f745a89b664e6bc2de64fb8e179e9717faf3569e))
* use bun runtime for sematic release ([f514766](https://github.com/kaiehrhardt/terradot/commit/f514766912420de072a9254d5dc1d55583335691))

## [1.2.3](https://github.com/kaiehrhardt/terradot/compare/1.2.2...1.2.3) (2026-04-10)


### Bug Fixes

* migrate to Tailwind CSS v4 PostCSS plugin and update CSS syntax ([91a8b05](https://github.com/kaiehrhardt/terradot/commit/91a8b05e56220d11428a0809055b5d2fe40d2aa0))

## [1.2.2](https://github.com/kaiehrhardt/terradot/compare/1.2.1...1.2.2) (2026-04-10)


### Bug Fixes

* **deps:** pin dependencies ([8f2e9de](https://github.com/kaiehrhardt/terradot/commit/8f2e9de19e34617284ce172b1ee6a655e2f2a704))

## [1.2.1](https://github.com/kaiehrhardt/terradot/compare/1.2.0...1.2.1) (2026-04-10)


### Bug Fixes

* **lint:** resolve CI lint errors in App.tsx and eslint config ([91f8988](https://github.com/kaiehrhardt/terradot/commit/91f898898736b4f96bddb336a61041bc874636c6))

# [1.2.0](https://github.com/kaiehrhardt/terradot/compare/1.1.1...1.2.0) (2026-02-28)


### Bug Fixes

* avoid loading overlay for successor toggle ([38a576e](https://github.com/kaiehrhardt/terradot/commit/38a576e18d0ca779686d04aeb2e306989fbd7eb9))


### Features

* add png/svg export ([a438856](https://github.com/kaiehrhardt/terradot/commit/a438856cb7aecb700dd3c029411bde4244b4e701))
* align panel controls and layout ([ddf3004](https://github.com/kaiehrhardt/terradot/commit/ddf300445f67833e896a9ea57846d520fc21816d))

## [1.1.1](https://github.com/kaiehrhardt/terradot/compare/1.1.0...1.1.1) (2026-02-27)


### Bug Fixes

* release trigger ([c750357](https://github.com/kaiehrhardt/terradot/commit/c7503570ae9cd763d097348046aa8bcc486dba0a))

# [1.1.0](https://github.com/kaiehrhardt/terradot/compare/1.0.0...1.1.0) (2026-02-27)


### Features

* restore header/footer details and version badge ([7656560](https://github.com/kaiehrhardt/terradot/commit/7656560c79548ddcb6d82476e6ff4301b2e7544a))

# 1.0.0 (2026-02-27)


### Bug Fixes

* add logos ([913ed3c](https://github.com/kaiehrhardt/terradot/commit/913ed3c4070af060a876d87697262f7d2f81d63b))
* switch default layout ([867cbe1](https://github.com/kaiehrhardt/terradot/commit/867cbe1ea5a77465a4f7dd5d8fcf5cc6d9cee786))


### Features

* add data node filter toggle ([62b8268](https://github.com/kaiehrhardt/terradot/commit/62b8268ba01d443f85c7d5dda519833298a47ca1)), closes [#3](https://github.com/kaiehrhardt/terradot/issues/3)
* add module ignore logic ([5bd177f](https://github.com/kaiehrhardt/terradot/commit/5bd177f9a4cd0c09e1923dd8c4d3267708591ee7))
* add predecessors and successors toggle + zoomscaleextent ([a4210eb](https://github.com/kaiehrhardt/terradot/commit/a4210ebddc78ee3a679c34ac6e010b7c3f8b61a7))
* add renovate ([02881ba](https://github.com/kaiehrhardt/terradot/commit/02881ba36bed14b97e4cce2ec3d94eefdf401d4c))
* add toggle for selection ([26b9549](https://github.com/kaiehrhardt/terradot/commit/26b95496fe9afc85a14bddec86753ade894bd488)), closes [#2](https://github.com/kaiehrhardt/terradot/issues/2)
* float selected node info ([9222e36](https://github.com/kaiehrhardt/terradot/commit/9222e367da30d3195b2c463216e0b3e8c583e05f))
* initial content ([3d2fa79](https://github.com/kaiehrhardt/terradot/commit/3d2fa79e098732851ca5a2b56773e1294467ed25))
* show graph loading overlay during renders ([d15ddc9](https://github.com/kaiehrhardt/terradot/commit/d15ddc9d1216243ce9dd1d018f6f08c653092684))
* some minor changes ([4c601d6](https://github.com/kaiehrhardt/terradot/commit/4c601d6aea3ff8c43564f72fcdaf28d820ff2022))
